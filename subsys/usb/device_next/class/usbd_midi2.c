...
static int midi1_ump_to_sysex_events(const struct midi_ump *const ump, const uint8_t cable_number,
				     uint32_t events[3])
{
	uint8_t status = UMP_DATA64_STATUS(*ump);
	uint8_t len = UMP_DATA64_LEN(*ump);
	uint8_t payload[6];
	uint8_t sysex[8];
	uint8_t total = 0;
	uint8_t processed = 0;
	...
	/* USB-MIDI 1.0 SysEx data bytes are 7-bit; reject values that cannot
...
int usbd_midi_send(const struct device *dev, const struct midi_ump ump)
{
	...
	if (IS_ENABLED(CONFIG_USBD_MIDI2_ALTSETTING_MIDI2) &&
		...
		if (UMP_MT(ump) == UMP_MT_DATA_64) {
			...
			needed = n_events * MIDI1_EVENT_BYTES;
		} else {
...
/* Total length of the USB-MIDI 1.0 class-specific MS interface header and
 * Jack descriptors. midi10 6.1.2.1: wTotalLength is the combined length of
 * this descriptor header and all Jack and Element descriptors; endpoint
